...
import 'package:lampfield/ui/mark.dart';
import 'package:lampfield/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real message, and checked', () {
    // The picture is lamps 3, 4, 5 and 6 alight: 3 and 4 and 5 and 6
    // come to 18, which is two nines, so it is in the code and the
    // reader mends whichever lamp goes out.
    final mark = Mark.lamps;
    expect(mark.message, [0, 0, 1, 1, 1, 1, 0, 0]);
    expect(Rules.valid(mark.message), isTrue);
    expect(mark.weight, 18);
    expect(mark.over9, 0);
    expect(mark.inCode, isTrue);
    expect(mark.lit, 4);
    expect(mark.mended, Rules.lamps);
    expect(mark.level.meets(mark.message), isTrue);
  });
