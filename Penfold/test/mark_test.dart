...
import 'package:penfold/ui/mark.dart';
import 'package:penfold/ui/palette.dart';
...
void main() {
  ...
  test('the mark is Cerny\'s fold, four whistles into its one call', () {
    // R L L L is the front of the only call of nine that gathers the
    // long fold.
    final play = Mark.partWay;
    expect(play.call, [1, 0, 0, 0]);
    expect(Rules.tellCall(play.call), 'R L L L');
    expect(play.spread, 3);
    expect(Rules.standing(play.flock), [0, 1, 2]);
    expect(play.isDone, isFalse);
    expect(play.away, 5);
    expect(Rules.fewest(play.whistles), 9);
  });
