...
import 'package:yokemere/ui/mark.dart';
import 'package:yokemere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the team that pulls hardest', () {
    // Both rows in matching order, which is the one yoking with nothing
    // crossed and so the one nothing can beat.
    final mark = Mark.team;
    expect(mark.order, [0, 1, 2, 3, 4]);
    expect(mark.pull, Rules.hardest());
    expect(mark.pull, 55);
    expect(mark.anyCrossed, isFalse);
    expect(mark.level.meets(mark.order), isTrue);
  });
