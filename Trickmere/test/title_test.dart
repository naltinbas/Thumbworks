import 'package:flutter_test/flutter_test.dart';
import 'package:trickmere/deck/levels.dart';

import 'support/fonts.dart';
import 'support/mereland.dart';

/// The sham, as first seen.
void main() {
  setUpAll(useRealFonts);

  testWidgets('the sham lists every hand by name and task',
      (tester) async {
    await open(tester);
    expect(find.text('Trickmere'), findsOneWidget);
    for (final level in Levels.all) {
      expect(find.text(level.name), findsOneWidget);
      expect(
        find.textContaining(level.task.substring(1)),
        findsWidgets,
      );
    }
  });

  testWidgets('a hand opens from its tile', (tester) async {
    await open(tester);
    await tester.tap(find.text('The Pair of Hearts'));
    await tester.pumpAndSettle();
    expect(
      find.textContaining('Tap a card to hide it'),
      findsOneWidget,
    );
  });

  testWidgets('a landing writes its fewest onto the sham',
      (tester) async {
    await open(tester);
    await tester.tap(find.text('The Pair of Hearts'));
    await tester.pumpAndSettle();
    await tapAll(tester, [32, 27, 12, 17, 47]);
    expect(state(tester).play.isDone, isTrue);
    await press(tester, 'The sham');
    expect(find.textContaining('Fewest: 5'), findsOneWidget);
  });
}
