...
import 'package:hoopwell/ui/mark.dart';
import 'package:hoopwell/ui/palette.dart';
...
void main() {
  ...
  test('the mark sits exactly on the floor', () {
    // Two dark stones and four pale, both runs at the same step, which
    // is the shape every board on the floor turns out to have.
    final mark = Mark.board;
    expect(Rules.at(mark.dark), [0, 1]);
    expect(Rules.at(mark.pale), [0, 1, 2, 3]);
    expect(mark.lampCount, 5);
    expect(mark.floor, 5);
    expect(Rules.isRun(mark.dark, 1), isTrue);
    expect(Rules.isRun(mark.pale, 1), isTrue);
    expect(mark.level.meets(mark.board), isTrue);
  });
