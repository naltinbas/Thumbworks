...
import 'package:flitwell/ui/mark.dart';
import 'package:flitwell/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the firm lane, and it is firm', () {
    // The picture is the one lane of the shared street that no group of
    // tenants can better, which the trading rings give.
    final mark = Mark.lane;
    expect(mark.level.name, 'The Firm Lane');
    expect(Rules.write(mark.where), 'BDCA');
    expect(Rules.firm(mark.orders, mark.where), isTrue);
    expect(Rules.settled(mark.orders, mark.where), isTrue);
    expect(mark.topped, [0, 1, 3]);
    expect(mark.level.meets(mark.where), isTrue);
  });
