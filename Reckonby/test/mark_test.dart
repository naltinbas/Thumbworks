...
import 'package:reckonby/ui/mark.dart';
import 'package:reckonby/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real reading, and checked', () {
    // The picture is every wheel at its top, which reads 719: the
    // highest the house goes, and one short of rolling over.
    final mark = Mark.wheels;
    expect(mark.wheels, [1, 2, 3, 4, 5]);
    expect(Rules.valid(mark.wheels), isTrue);
    expect(mark.reading, Rules.most);
    expect(mark.reading, 719);
    expect(mark.under, 0);
    expect(Rules.tickUp(mark.wheels), isNull);
    expect(mark.level.meets(mark.wheels), isTrue);
  });
