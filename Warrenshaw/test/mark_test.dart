...
import 'package:warrenshaw/ui/mark.dart';
import 'package:warrenshaw/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a chase one move from the end, not a picture of one', () {
    // Played out through the table and the same code a finger goes through,
    // and stopped where the picture is worth something: one move to go.
    final play = Mark.nearlyOver;
    expect(play.left, 1);
    expect(play.isDone, isFalse);
    expect(play.moves, greaterThan(0));
    expect(play.chart.beside[play.seeker], contains(play.runner),
        reason: 'and it is cornered: one path away, with nowhere to go');
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onField: false),
        ),
      );
    }
