...
import 'package:roostwick/ui/mark.dart';
import 'package:roostwick/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real wood, and settled', () {
    // The picture is the hub settled: six birds in six hollows, with the
    // doubled tether between A and B turned one way.
    final mark = Mark.wood;
    expect(mark.level.name, 'The Hub');
    expect(mark.at, [0, 1, 2, 3, 4, 5]);
    expect(mark.crowded, isEmpty);
    expect(mark.level.meets(mark.pick), isTrue);
    expect(Rules.tally(mark.birds), 2);
    expect(Rules.census(mark.birds), 2);
  });
