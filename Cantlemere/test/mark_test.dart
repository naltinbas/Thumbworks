...
import 'package:cantlemere/ui/mark.dart';
import 'package:cantlemere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real three-plot cut', () {
    // Every cut of the field into three plots comes out 3, 6 and 9 half
    // acres with exactly one motley plot, and this is one of the 32.
    final mark = Mark.cut;
    expect(mark.laid.length, 3);
    expect(mark.sizes..sort(), [3, 6, 9]);
    expect(mark.motley.length, 1);
    expect(mark.taken, Rules.field);
    expect(Rules.cuts(mark.pieces), isTrue);
  });
