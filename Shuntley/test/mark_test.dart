...
import 'package:shuntley/ui/mark.dart';
import 'package:shuntley/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the morning shunt, ten from home', () {
    // The pictured tray goes through the same code a finger goes
    // through, so the logo is a live game, not an arrangement.
    var play = Mark.dealt;
    expect(play.fewestFromHere, 10);
    var guard = 0;
    while (!play.isHome && guard++ < 12) {
      play = play.shunt(play.next!);
    }
    expect(play.isHome, isTrue);
  });
