...
///
/// Every clock in them was wound to by taps, so nothing in the pictures
/// is a clock the game could not reach.
///
...
void main() {
  ...
  for (final phone in phones.entries) {
    ...
    testWidgets('the sixty on ${phone.key}', (tester) async {
      await show(tester, phone.value, which: 2);
      await setClock(tester, 10);
      expect(state(tester).play.isDone, isTrue);
      await shoot(tester, 'sixty-${phone.key}');
    });
  }

  testWidgets('the eight', (tester) async {
    await show(tester, phones['iphone-14']!, which: 0);
    await setClock(tester, 3);
    expect(state(tester).play.isDone, isTrue);
  ...
  testWidgets('the odd period admitted', (tester) async {
    await show(tester, phones['iphone-14']!, which: 4);
    await setClock(tester, 4);
    expect(state(tester).play.gaveUp, isTrue);
    await shoot(tester, 'oddperiod');
  });
