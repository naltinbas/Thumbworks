...
import 'package:packwold/ui/mark.dart';
import 'package:packwold/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a box that is really packed', () {
    // It is drawn by the game rather than for it, so this is worth saying
    // out loud: the picture is a box with nothing bare left on it.
    expect(Mark.packed.isDone, isTrue);
    expect(Mark.packed.empty, 0);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onChalk: false),
        ),
      );
    }
