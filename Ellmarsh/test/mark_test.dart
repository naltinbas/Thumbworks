...
import 'package:ellmarsh/ui/mark.dart';
import 'package:ellmarsh/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the first bench, past the golden tick', () {
    // The bolts come through the same painter the why uses, and the
    // long bolt reaches past the gap: a bench the cutter holds.
    final play = Mark.standing;
    expect(play.winnable, isTrue);
    expect(play.long * play.long,
        greaterThan(play.long * play.short + play.short * play.short));
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onBench: false),
        ),
      );
    }
