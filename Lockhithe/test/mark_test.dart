...
import 'package:lockhithe/ui/mark.dart';
import 'package:lockhithe/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a stow whose ropes cover every locker once', () {
    // The loops come through the same painter the why uses.
    final play = Mark.roped;
    final counted = play.stow.loops.fold(0, (sum, loop) => sum + loop.length);
    expect(counted, play.berth.lockers);
    expect(play.stow.longestLoop, 3);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onWharf: false),
        ),
      );
    }
