...
/// One ask, the gaps on their dials and the waits they make.
class WaitScreen extends StatefulWidget {
  const WaitScreen({super.key, required this.level});
...
class WaitScreenState extends State<WaitScreen> {
  ...
  String verdict() {
    ...
    final head = 'Gaps ${Rules.tellGaps(play.gaps)}: average wait ${Rules.tell(play.wait)} minutes, ${play.wait == Rules.fairWait ? 'the fair wait itself' : '${Rules.tell(play.over)} over the fair 9 1/2'}.';
    if (play.gaveUp) return '$head Never under, whatever the gaps.';
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Text(
                  'Step the first two gaps up and down, the third what is '
                  'left of the hour, and watch the average wait: '
                  '${widget.level.task}.',
                  style: const TextStyle(
              ...
              if (!play.isOver)
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      _dial('g1', 'gap 1', play.first),
                      const SizedBox(width: 8),
                      _dial('g2', 'gap 2', play.second),
                    ],
                  ),
                ),
              if (!play.isOver)
                ...
                children: [
                  ...
                  Chip(
                    ...
                    side: BorderSide(
                        color: play.isDone ? Palette.good : Palette.line),
                    label: Text(
                      play.wait == Rules.fairWait ? 'the fair wait' : '${Rules.tell(play.over)} over fair',
                      style: const TextStyle(
