...
import 'package:frogmere/ui/mark.dart';
import 'package:frogmere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the third reach two leaps in, real and checked', () {
    // The picture is the eight-frog cross with the count's first
    // two leaps taken, a frog above the reeds and the weight
    // still exactly one.
    final play = Mark.thirdReach;
    expect(play.frogs, hasLength(6));
    expect(play.weight, closeTo(1, 1e-12));
    expect(play.frogs.any((f) => f.$2 >= 1), isTrue);
  });
