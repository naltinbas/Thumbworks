...
# for are the point: each is taken at a chosen moment of a game the test
# played itself, so the one that matters is a game played out against the house.
...
package=dev.hazardwell.hazardwell
