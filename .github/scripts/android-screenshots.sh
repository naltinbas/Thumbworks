#!/usr/bin/env bash
#
# Solves a puzzle on the emulator the runner has already booted, and keeps the
# pictures.
#
# Two kinds of picture come out of a run. The ones the test asks the framework
# for are the point: each is taken at a chosen moment in a puzzle the test
# solved itself, so the one that matters has a thumb part way along a run of
# squares it is filling in. The ones sampled off the
...
package=dev.tallyloom.tallyloom
