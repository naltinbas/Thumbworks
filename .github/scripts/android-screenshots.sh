#!/usr/bin/env bash
#
# Takes a run on the emulator the runner has already booted, and keeps the
# pictures.
#
...
# played itself, so the one that matters has the runner in the air over a
# gap. The ones sampled off the
...
package=dev.vaultline.vaultline
