#!/usr/bin/env bash
#
# Plays a round on the emulator the runner has already booted, and keeps the
# pictures.
#
# Two kinds of picture come out of a run. The ones the test asks the framework
# for are the point: each is taken at a chosen moment in a round the test
# played itself, so the one that matters has a thumb part way along a word
# with the game already saying the letters count. The ones sampled off the
...
package=dev.latchword.latchword
