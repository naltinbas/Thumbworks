#!/usr/bin/env bash
#
# Plays a deal on the emulator the runner has already booted, and keeps the
# pictures.
#
# Two kinds of picture come out of a run. The ones the test asks the framework
# for are the point: each is taken at a chosen moment in a deal the test
# played itself, so the one that matters has the hint pointing at a card and
# at where it is going. The ones sampled off the
...
package=dev.fanwright.fanwright
