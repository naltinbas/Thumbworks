#!/usr/bin/env bash
#
# Plays a tune on the emulator the runner has already booted, and keeps the
# pictures.
#
# Two kinds of picture come out of a run. The ones the test asks the framework
# for are the point: each is taken at a chosen moment in a tune the test
# played itself, so the one that matters is a note landing on the line with
# the game saying so. The ones sampled off the
