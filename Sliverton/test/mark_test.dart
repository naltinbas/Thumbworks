...
import 'package:sliverton/ui/mark.dart';
import 'package:sliverton/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the seventh, real and checked', () {
    // The picture is the marks 8, 8 and 8: the cuts run to the two-thirds
    // mark on every side and the sliver is a seventh of the field.
    final play = Mark.seventh;
    expect(play.marks, [8, 8, 8]);
    expect(play.share, Frac.of(1, 7));
    expect(play.shareByRouth, Frac.of(1, 7));
    expect(play.cutsMeet, isFalse);
    expect(play.sliver!.map(Rules.tellSpot).toList(), ['(12/7, 24/7)', '(48/7, 12/7)', '(24/7, 48/7)']);
