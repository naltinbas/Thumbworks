...
import 'package:colthorpe/ui/mark.dart';
import 'package:colthorpe/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the five yard nine true jumps in', () {
    // Every jump follows the walk through the same code a finger goes
    // through, so the picture is a round that can still be ridden home.
    final play = Mark.nineIn;
    expect(play.path, hasLength(9));
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onStable: false),
        ),
      );
    }
