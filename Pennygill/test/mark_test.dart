...
import 'package:pennygill/ui/mark.dart';
import 'package:pennygill/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the ring, every call with its beater', () {
    // The ring comes through the same painter the why uses, and every
    // arrow points from a call to the call that beats it.
    final play = Mark.ringed;
    expect(play.called, isTrue);
    expect(play.theirs, play.yours!.beatenBy);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onBoard: false),
        ),
      );
    }
