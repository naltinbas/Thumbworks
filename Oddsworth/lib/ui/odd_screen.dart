...
/// One ask, the run set to it, the square of dots drawn.
class OddScreen extends StatefulWidget {
  const OddScreen({super.key, required this.level});

  final Level level;

  @override
  State<OddScreen> createState() => OddScreenState();
}

class OddScreenState extends State<OddScreen> {
  late Play play;
  ...
  String verdict() {
    ...
    final head = '$told = ${play.sum}: ${play.inner == 0 ? '${play.count} squared' : '${play.outer} squared less ${play.inner} squared'}, ${play.sum % 4 == 0 ? 'a multiple of four' : play.sum.isOdd ? 'odd' : 'even'}.';
    if (play.gaveUp) return '$head Thirty is two past a multiple of four, and no run makes such a number.';
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Text(
                  'Set the first odd number and how many follow it, and see '
                  'the run laid as bands of dots round a square: '
                  '${widget.level.task}.',
                  style: const TextStyle(
              ...
              if (!play.isOver)
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      _dial(0, 'first', 'first ${play.first}'),
                      const SizedBox(width: 8),
                      _dial(1, 'count', 'count ${play.count}'),
                    ],
                  ),
                ),
              if (!play.isOver)
                ...
                children: [
                  Chip(
                    ...
                    label: Text(
                      'sum ${play.sum}',
                      style: const TextStyle(
                          color: Palette.bandB, fontSize: 13),
                    ),
                  ),
                  Chip(
                    backgroundColor: Palette.board,
                    side: BorderSide(
                        color: play.isDone ? Palette.good : Palette.line),
                    label: Text(
                      '${play.outer} squared less ${play.inner} squared',
                      style: const TextStyle(
