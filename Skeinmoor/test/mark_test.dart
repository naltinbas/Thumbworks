...
import 'package:skeinmoor/ui/mark.dart';
import 'package:skeinmoor/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a board that is really filled', () {
    // It is drawn by the game rather than for it, so this is worth saying
    // out loud: the picture is a board with nothing left over on it.
    expect(Mark.filled.isDone, isTrue);
    expect(Mark.filled.empty, 0);
  });
  ...
  testWidgets('and the iOS ones, every size the app icon set asks for',
    ...
    for (final image in listed['images'] as List<dynamic>) {
      ...
      final scale =
          double.parse((what['scale'] as String).replaceAll('x', ''));
