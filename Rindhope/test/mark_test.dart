...
import 'package:rindhope/ui/mark.dart';
import 'package:rindhope/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the square two exchanges in, still winning', () {
    // Every bite runs through the same code a finger goes through, so
    // the picture is a block still on course for its fewest.
    final play = Mark.twoIn;
    expect(play.made, 2);
    expect(play.couldFinishIn, play.block.fewest);
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onShelf: false),
        ),
      );
    }
