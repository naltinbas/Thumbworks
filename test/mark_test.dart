...
import 'package:chalkway/ui/mark.dart';
import 'package:chalkway/ui/palette.dart';
...
void main() {
  ...
  testWidgets('the app icon', (tester) async {
    await draw(
      ...
      const ColoredBox(
        color: Palette.slateDeep,
        child: Padding(padding: EdgeInsets.all(56), child: Mark()),
      ),
    );
  ...
  testWidgets('the adaptive icon foreground, on nothing', (tester) async {
    // Android puts this on a layer of its own and then crops it to whatever
    // shape the launcher likes, so it is drawn small and well inside the edge.
    await draw(
      tester,
      'icon-foreground',
      1024,
