...
past n that the rope covers. Bertrand's postulate says the numbers can
never strand you: for every n above 1 there is a prime p with
n < p < 2n. The ford still can, since it stops at 120: from stone 113
the next dry stone, 127, is past its end. Joseph Bertrand stated the
postulate in 1845, in the stronger form n < p < 2n - 2, after checking
the numbers up to three million; Pafnuty Chebyshev proved it in 1850
and printed the proof in 1852; Srinivasa Ramanujan gave a simpler
proof in 1919, and Paul Erdos a short elementary one in 1932, his
first paper in a research journal. What the postulate does not say is
where the dry stone will be, and that is the game. The checker sieves
every stone of the ford twice, once by Eratosthenes and once by trial
division, asks every number up to 200,000 for a dry stone inside its
rope, and walks the ford for the fewest hops each ask takes.
...
3. **The Far Bank** - cross to a dry stone whose rope reaches past the ford's last
...
reached from stone 2: 3 in a hop, then 5, 7, 13, 23, 43, 83, and by
...
cases are settled in the modern proof, and the greedy one, each stone
the farthest the rope covers, turns out to be the frontier of the
fewest-hop walk: the stones you can reach in d hops or fewer are
exactly the dry ones up to where the chain stands after d hops. Five dry stones
...
Long Shallows is washed red on the board rather than gold, and the
reason fits on a finger: the seven stones from 90 to 96 are all mossy, since 90, 92,
...
* **Trial division and the chain** come at it from the other end.
  Dryness is settled by dividing, which never looks at the sieve, and
  the fewest hops by the greedy chain, which never searches the hop
  graph: 2, 3, 5, 7, 13, 23, 43, 83, 163, 317, 631, 1259, 2503, and
  on. The two agree on every stone and on every hop
...
one, and from stone 2 on the stone that uses the most of its rope is
...
```
every number from 1 to 200,000 asked for a dry stone inside the rope's reach and given one, the first above it found twice, by a pointer walking the sieve and, up to 20,000, by trial division counting up from n + 1, the two agreeing on every one: the promise never fails, and from stone 2 on the stone that uses the most of its rope is 3, where the next dry stone is 5 and the rope reaches 6; the strict form n < p < 2n holds from 2 up and fails at 1 alone, where the only stone in reach is the rope's own end; the greedy crossing, always the farthest stone the rope covers, runs 2, 3, 5, 7, 13, 23, 43, 83, 163, 317, 631, 1259, 2503, 5003, 9973, 19937, 39869, 79699, 159389, and a walk over the hop graph of the 2,262 dry stones below 20,000, stone by stone and chain unseen, gives the same counts: 8 hops to pass a hundred, 11 a thousand, 15 ten thousand, and the stones reachable in d hops or fewer are exactly the dry ones up to where the chain stands after d hops; on the ford itself, 120 stones with 30 dry, sieved and divided out and agreeing on every one, every dry stone can be reached and none takes more than 8 hops: 1 at 0, 1 at 1, 1 at 2, 1 at 3, 2 at 4, 3 at 5, 5 at 6, 9 at 7, 7 at 8; 5 dry stones lie past the hundredth, 101, 103, 107, 109, 113, the rope runs past the ford's last stone from 61 on, 13 of them, 10 stones have another dry stone two behind, and 2 have nothing but moss for four either side, 53 and 89; the seven stones from 90 to 96 are all mossy, the first such run anywhere, stone 91 is 7 times 13 and stone 93 is 3 times 31, so no crossing ends in the shallows
 ...
 3 The Far Bank      cross to a dry stone whose rope reaches past the ford's last: 13 of the 120 stones land it, the fewest crossing 7 hops
...
every hop in them was made by a tap on the stone, so no crossing
pictured is one the game could not make. The logo and every launcher
