...
import 'package:ropeford/ui/mark.dart';
import 'package:ropeford/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real crossing, hop by hop', () {
    // The picture is the greedy crossing five hops in: 2, 3, 5, 7, 13, 23,
    // each stone dry and each one inside the rope from the stone before.
    final play = Mark.crossing;
    expect(play.stones, [2, 3, 5, 7, 13, 23]);
    for (var i = 1; i < play.stones.length; i++) {
      expect(Rules.canHop(play.stones[i - 1], play.stones[i]), isTrue,
          reason: '${play.stones[i - 1]} to ${play.stones[i]}');
    }
    expect(play.stones, Rules.chain.take(6));
    expect(play.at, 23);
    expect(play.rope, 46);
    expect(play.inReach, [29, 31, 37, 41, 43]);
    expect(Rules.hops[23], 5);
  });
