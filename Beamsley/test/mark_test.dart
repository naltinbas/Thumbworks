...
import 'package:beamsley/ui/mark.dart';
import 'package:beamsley/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the named casting, real and checked', () {
    // The picture is the pegs (2, -2), (0, 2) and (2, 1) cast 2, -2 and
    // -1: the sides meet at (3, -4), (-6, 5) and (2, -3), three peg
    // places on the line x + y = -1.
    final play = Mark.cast;
    expect(play.pegs, [(2, -2), (0, 2), (2, 1)]);
    expect(play.casts, [2, -2, -1]);
    expect(play.shadows, [(4, -4), (0, -4), (-2, -1)]);
    expect(play.meetings!.map(Rules.tellPoint).toList(), ['(3, -4)', '(-6, 5)', '(2, -3)']);
    expect(play.inLine, isTrue);
    expect(Rules.tellLine(play.axis!), 'the line x + y = -1');
