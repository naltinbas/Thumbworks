...
import 'package:plaitwell/ui/mark.dart';
import 'package:plaitwell/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the trefoil painted in all three dyes', () {
    // The first knot there is, plaited on two ropes with three crossings,
    // and painted so every crossing shows all three colours.
    final mark = Mark.plait;
    expect(mark.level.strands, 2);
    expect(mark.level.word, [1, 1, 1]);
    expect(mark.paint, [0, 1, 2]);
    expect(mark.paint.length, mark.level.arcs);
    expect(mark.allSound, isTrue);
    expect(mark.shades, Rules.colours);
    expect(mark.level.meets(mark.paint), isTrue);
  });
