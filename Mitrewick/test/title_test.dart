import 'package:flutter_test/flutter_test.dart';
import 'package:mitrewick/board/levels.dart';

import 'support/fonts.dart';
import 'support/wickland.dart';

/// The sham, as first seen.
void main() {
  setUpAll(useRealFonts);

  testWidgets('the sham lists every board by name and task',
      (tester) async {
    await open(tester);
    expect(find.text('Mitrewick'), findsOneWidget);
    for (final level in Levels.all) {
      expect(find.text(level.name), findsOneWidget);
      expect(
        find.textContaining(level.task.substring(1)),
        findsWidgets,
      );
    }
  });

  testWidgets('a board opens from its tile', (tester) async {
    await open(tester);
    await tester.tap(find.text('The Three'));
    await tester.pumpAndSettle();
    expect(
      find.textContaining('Tap squares to set bishops'),
      findsOneWidget,
    );
  });

  testWidgets('a landing writes its fewest onto the sham',
      (tester) async {
    await open(tester);
    await tester.tap(find.text('The Three'));
    await tester.pumpAndSettle();
    await setAll(tester, [(0, 0), (0, 1), (0, 2), (2, 1)]);
    expect(state(tester).play.isDone, isTrue);
    await press(tester, 'The sham');
    expect(find.textContaining('Fewest: 4'), findsOneWidget);
  });
}
