...
import 'package:caskleigh/ui/mark.dart';
import 'package:caskleigh/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real run of casks, and checked', () {
    // The picture is the first eleven casks, which fill three barrels
    // and a sliver over, with the eighth cask lit because it holds
    // three twos and nothing else in the run holds as many.
    final play = Mark.run;
    expect((play.first, play.last), (1, 11));
    expect(play.total.toString(), '83711/27720');
    expect(play.total.toDouble.ceil(), 4);
    expect(play.deepest, [8]);
    expect(Rules.mostTwos(1, 11), 3);
    expect(play.level.meets(1, 11), isTrue);
  });
