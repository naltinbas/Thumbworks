...
/// One ask, the three bends on their dials and the bubbles they make.
class KissScreen extends StatefulWidget {
  const KissScreen({super.key, required this.level});

  final Level level;

  @override
  State<KissScreen> createState() => KissScreenState();
}

class KissScreenState extends State<KissScreen> {
  late Play play;
  ...
  String verdict() {
    ...
    final head = 'Bends ${Rules.tell(play.bends)}: ${play.inner} in the gap, ${play.outer} ${outerWords(play)}${play.whole ? ', both whole' : ''}.';
    if (play.gaveUp) return '$head Apart, whatever the bends.';
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Text(
                  'Step the three bends, one to twenty, and watch the bubble '
                  'in the gap and the one round the outside: '
                  '${widget.level.task}.',
                  style: const TextStyle(
