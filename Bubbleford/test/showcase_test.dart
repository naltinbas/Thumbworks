...
void main() {
  ...
  for (final phone in phones.entries) {
    ...
    testWidgets('the unit ring on ${phone.key}', (tester) async {
      await show(tester, phone.value, which: 0);
      await setBends(tester, [2, 2, 3]);
      expect(state(tester).play.isDone, isTrue);
      await shoot(tester, 'unit-${phone.key}');
    });
  }

  testWidgets('the flat fourth', (tester) async {
  ...
  testWidgets('the whole wrap', (tester) async {
    await show(tester, phones['iphone-14']!, which: 2);
    await setBends(tester, [2, 3, 6]);
    expect(state(tester).play.isDone, isTrue);
    await shoot(tester, 'wrap');
  });

  testWidgets('the far gap', (tester) async {
    await show(tester, phones['iphone-14']!, which: 3);
    await setBends(tester, [1, 1, 12]);
    expect(state(tester).play.isDone, isTrue);
    await shoot(tester, 'gap');
  });
  ...
  testWidgets('the twin fourths admitted', (tester) async {
    await show(tester, phones['iphone-14']!, which: 4);
    for (var k = 0; k < 12; k++) {
