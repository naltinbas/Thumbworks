...
///
/// Every crossing in them was set by taps on the pegs, so nothing in
/// the pictures is a crossing the game could not reach.
///
...
void main() {
  ...
  for (final phone in phones.entries) {
    ...
    testWidgets('the halved on ${phone.key}', (tester) async {
      await show(tester, phone.value, which: 3);
      await setPegs(tester, [1, 5, 3, 9]);
      expect(state(tester).play.isDone, isTrue);
      await shoot(tester, 'halved-${phone.key}');
    });
  }
  ...
  testWidgets('the odd cross admitted', (tester) async {
    await show(tester, phones['iphone-14']!, which: 4);
    await setPegs(tester, [0, 4, 1, 6]);
    expect(state(tester).play.gaveUp, isTrue);
    await shoot(tester, 'oddcross');
  });
