...
import 'package:braidfell/ui/mark.dart';
import 'package:braidfell/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the doubles, two braids in', () {
    // The mark goes through the same code a finger goes through, so
    // the picture is a yard a test braids to its asking.
    final play = Mark.opening;
    expect(play.yard.name, 'The Doubles');
    expect(play.bundles, hasLength(3));
    expect(play.work, 10);
    expect(play.floor, play.yard.least);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onPanel: false),
        ),
      );
    }
