...
import 'package:reelbury/ui/mark.dart';
import 'package:reelbury/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a round that really holds', () {
    // Put together through the same code a finger goes through, so the three
    // lines in the picture are three couples nobody would swap out of.
    final play = Mark.paired;
    expect(play.isFull, isTrue);
    expect(play.blocking, isEmpty);
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onFloor: false),
        ),
      );
    }
