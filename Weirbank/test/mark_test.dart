...
import 'package:weirbank/ui/mark.dart';
import 'package:weirbank/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a works really running as full as it can', () {
    // Put through the same code a finger goes through, so the water in the
    // picture obeys every pipe and every pond.
    final play = Mark.running;
    expect(play.holds, isTrue);
    expect(play.isDone, isTrue);
    expect(play.arriving, Mark.waterwork.target);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onStone: false),
        ),
      );
    }
