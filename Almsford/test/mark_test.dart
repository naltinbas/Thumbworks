...
import 'package:almsford/ui/mark.dart';
import 'package:almsford/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real arrangement, and checked', () {
    // The picture is the staircase, 0, 1, 2, 3, 4: ten measures in
    // five bins, every height different.
    final mark = Mark.bins;
    expect(mark.bins, [0, 1, 2, 3, 4]);
    expect(Rules.valid(mark.bins), isTrue);
    expect(mark.shape, [4, 3, 2, 1, 0]);
    expect(mark.running, [4, 7, 9, 10, 10]);
    expect(mark.holding, isNull);
    expect(mark.level.meets(mark.bins), isTrue);
    expect(Rules.covers(Rules.opening, mark.shape), isTrue);
  });
