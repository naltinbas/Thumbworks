...
import 'package:truckleford/ui/mark.dart';
import 'package:truckleford/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real yard, and reachable', () {
    // The picture is four taps into a run: wagons 1, 2 and 3 shunted
    // onto the siding and wagon 4 rolled straight out.
    final mark = Mark.yard;
    expect(mark.line, [5, 6]);
    expect(mark.siding, [1, 2, 3]);
    expect(mark.out, [4]);
    var walked = Play.of(mark.level);
    for (final what in [Rules.shunt, Rules.shunt, Rules.shunt, Rules.roll]) {
      walked = walked.tap(what);
    }
    expect(walked.line, mark.line);
    expect(walked.siding, mark.siding);
    expect(walked.out, mark.out);
    expect(walked.wedged, isFalse);
  });
