...
import 'package:staddlestone/ui/mark.dart';
import 'package:staddlestone/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the half way point of a shortest way', () {
    // Every move follows the table through the same code a finger goes
    // through, so the picture is the moment the biggest stone crosses: half
    // the moves spent, half and one less to come, still at par.
    final play = Mark.halfWay;
    expect(play.biggestHome, isTrue);
    expect(play.made, 1 << (play.yard.stones - 1));
    expect(play.couldFinishIn, play.yard.fewest);
  });
