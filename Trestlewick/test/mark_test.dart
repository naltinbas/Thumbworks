...
import 'package:trestlewick/ui/mark.dart';
import 'package:trestlewick/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a site really part raised', () {
    // Raised through the same code a finger goes through, so the picture is a
    // frame the game could actually be standing on: three days gone, nothing
    // up before what it rests on, and the crews already set on for the next.
    final play = Mark.part;
    expect(play.day, 3);
    expect(play.isDone, isFalse);
    expect(play.today, isNotEmpty);
    for (var timber = 0; timber < Mark.frame.count; timber++) {
      if (!play.isUp(timber)) continue;
      for (final on in Mark.frame.rests[timber]) {
        expect(play.isUp(on), isTrue);
      }
    }
  });
