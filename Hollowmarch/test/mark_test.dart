...
import 'package:hollowmarch/ui/mark.dart';
import 'package:hollowmarch/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a game part way through, not a picture of one', () {
    // Three jumps into the fewest way through a real board, with the last
    // peg still on the move and able to take again — which is the one thing
    // about this game worth showing in a picture.
    final play = Mark.part;
    expect(play.jumps, hasLength(3));
    expect(play.left, Mark.board.hollows - 4);
    expect(play.carrying, isNonNegative);
    expect(play.canJump, isNotEmpty, reason: 'and it can carry on');
  });
