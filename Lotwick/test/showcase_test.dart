...
void main() {
  ...
  for (final phone in phones.entries) {
    ...
    testWidgets('the windfall on ${phone.key}', (tester) async {
      await show(tester, phone.value, which: 1);
      await setByPointer(tester);
      expect(state(tester).play.isDone, isTrue);
      await shoot(tester, 'windfall-${phone.key}');
    });
  }

  testWidgets('the overbid loss', (tester) async {
    await show(tester, phones['iphone-14']!, which: 0);
    await setDials(tester, 10, 12, 11);
    expect(state(tester).play.isDone, isTrue);
    await shoot(tester, 'overbid');
  });

  testWidgets('the sale passed up', (tester) async {
    await show(tester, phones['iphone-14']!, which: 2);
    await setDials(tester, 12, 11, 11);
    expect(state(tester).play.isDone, isTrue);
    await shoot(tester, 'passed');
  });

  testWidgets('the shading gain', (tester) async {
    await show(tester, phones['iphone-14']!, which: 3);
    await setDials(tester, 12, 11, 10);
    expect(state(tester).play.isDone, isTrue);
    await shoot(tester, 'shading');
  });

  testWidgets('midway, on the small phone', (tester) async {
    await show(tester, phones['iphone-se']!, which: 4);
    await setDials(tester, 9, 12, 12);
  ...
  testWidgets('outbidding the truth given up', (tester) async {
    await show(tester, phones['iphone-14']!, which: 4);
    await setDials(tester, 8, 8, 8);
    expect(state(tester).play.gaveUp, isTrue);
    await shoot(tester, 'nobeat');
  });
