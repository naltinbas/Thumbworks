...
import 'package:rimeworth/ui/mark.dart';
import 'package:rimeworth/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a run really driven', () {
    // Driven through the same code a finger goes through, so the picture is a
    // parish the game could actually be part way through: one run, no lane
    // salted twice, and the lorry out in the middle of it with two lanes left
    // to do.
    final play = Mark.part;
    expect(play.runs, 1);
    expect(play.isDone, isFalse);
    expect(play.at, isNonNegative);
    expect(play.salted.toSet(), hasLength(play.done));
    expect(play.done, Mark.parish.laneCount - 2);
  });
