...
import 'package:tosswell/ui/mark.dart';
import 'package:tosswell/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real rule, and checked', () {
    // The picture is the rule that takes a shilling after the first
    // toss and again after the third: ahead on 22 of the 32 runs, the
    // most there is, and still averaging nothing.
    final mark = Mark.rule;
    expect(mark.stop, {Rules.mark((1, 1)), Rules.mark((3, 1))});
    expect(mark.ahead, 22);
    expect(mark.added, 0);
    expect(mark.worst, -5);
    expect(mark.level.meets(mark.stop), isTrue);
  });
