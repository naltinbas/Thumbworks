...
///
/// Every king in them was set by taps, so nothing in the pictures is
...
void main() {
  ...
  testWidgets('a board mid-setting, a clash', (tester) async {
    await show(tester, phones['iphone-14']!, which: 1);
    await tapAll(tester, [0, 1, 10]);
    expect(state(tester).play.clashes, [(0, 1)]);
  ...
  testWidgets('the five admitted, kings touching',
      (tester) async {
    await show(tester, phones['iphone-14']!, which: 4);
    await tapAll(tester, [0, 2, 8, 10, 5, 5, 5, 5, 5, 5, 5, 5, 5]);
