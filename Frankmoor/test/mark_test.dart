...
import 'package:frankmoor/ui/mark.dart';
import 'package:frankmoor/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the first letter, paid to the penny', () {
    // Every stamp went on through the same code a finger goes through.
    final play = Mark.paid;
    expect(play.isPaid, isTrue);
    expect(play.cheaps, 2);
    expect(play.dears, 2);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onCounter: false),
        ),
      );
    }
