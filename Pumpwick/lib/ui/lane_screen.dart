...
class LaneScreenState extends State<LaneScreen> {
  ...
  /// What the sham says of the pump, as it stands.
  String verdict() {
    final aim = pointing;
    if (aim != null) return Play.pointed(aim);
    final head = 'The pump at spot ${play.spot}, and the walking comes to '
        '${play.walk}.';
    if (play.gaveUp) return '$head Nothing on the lane goes under it.';
    if (play.isDone) return 'As asked. $head';
    return '$head ${stepChip(play)}.';
  }
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Text(
                  'Tap along the lane to roll the pump that way, a spot at a '
                  'time: ${widget.level.task}.',
                  style:
