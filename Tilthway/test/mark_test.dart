...
import 'package:tilthway/ui/mark.dart';
import 'package:tilthway/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the eight seeds, and they can still come home', () {
    // The pictured board goes through the same code a finger goes
    // through, so the logo is a live game, not an arrangement.
    final play = Mark.grown;
    expect(play.board, [0, 2, 2, 4]);
    expect(play.canStill, isTrue);
    expect(play.trapped, isEmpty);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onBarnwood: false),
        ),
      );
    }
