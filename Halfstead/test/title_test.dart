import 'package:flutter_test/flutter_test.dart';
import 'package:halfstead/step/levels.dart';

import 'support/fonts.dart';
import 'support/stepland.dart';

/// The sham, as first seen.
void main() {
  setUpAll(useRealFonts);

  testWidgets('the sham lists every ask by name and task',
      (tester) async {
    await open(tester);
    expect(find.text('Halfstead'), findsOneWidget);
    for (final level in Levels.all) {
      expect(find.text(level.name), findsOneWidget);
      expect(
        find.textContaining(level.task.substring(1)),
        findsWidgets,
      );
    }
  });

  testWidgets('an ask opens from its tile', (tester) async {
    await open(tester);
    await tester.tap(find.text('The Quarter Left'));
    await tester.pumpAndSettle();
    expect(
      find.textContaining('Set the share of what is left each step covers'),
      findsOneWidget,
    );
  });

  testWidgets('a run writes its fewest onto the sham',
      (tester) async {
    await open(tester);
    await tester.tap(find.text('The Quarter Left'));
    await tester.pumpAndSettle();
    await turn(tester, 'steps', 1);
    expect(state(tester).play.isDone, isTrue);
    await press(tester, 'The sham');
    expect(find.textContaining('Fewest: 1'), findsOneWidget);
  });
}
