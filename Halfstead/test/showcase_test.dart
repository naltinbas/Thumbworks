...
void main() {
  ...
  for (final phone in phones.entries) {
    ...
    testWidgets('the hundredth on ${phone.key}', (tester) async {
      await show(tester, phone.value, which: 0);
      await setDials(tester, 0, 7);
      expect(state(tester).play.isDone, isTrue);
      await shoot(tester, 'hundredth-${phone.key}');
    });
  }

  testWidgets('the quarter left', (tester) async {
    await show(tester, phones['iphone-14']!, which: 1);
    await setDials(tester, 3, 1);
    expect(state(tester).play.isDone, isTrue);
  ...
  testWidgets('the thousandth by tenths', (tester) async {
    ...
    expect(state(tester).play.isDone, isTrue);
    await shoot(tester, 'thousandth');
  });

  testWidgets('the sixty-fourth', (tester) async {
    await show(tester, phones['iphone-14']!, which: 3);
    await setDials(tester, 0, 6);
    expect(state(tester).play.isDone, isTrue);
    await shoot(tester, 'sixtyfourth');
  });

  testWidgets('midway, on the small phone', (tester) async {
    await show(tester, phones['iphone-se']!, which: 0);
    await setDials(tester, 0, 4);
  ...
  testWidgets('the wall admitted', (tester) async {
    await show(tester, phones['iphone-14']!, which: 4);
    await setDials(tester, 0, 20);
    expect(state(tester).play.gaveUp, isTrue);
    await shoot(tester, 'wall');
  });
