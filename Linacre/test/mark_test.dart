...
import 'package:linacre/ui/mark.dart';
import 'package:linacre/ui/palette.dart';
...
void main() {
  ...
  test('the mark is one real exchange on the toll bridge', () {
    // The cut is the winning first cut and the brace is the machine's true
    // answer, both through the same code a finger goes through, and the win
    // is still whole.
    final play = Mark.oneExchange;
    expect(play.made, 1);
    expect(play.cut != 0, isTrue);
    expect(play.braced != 0, isTrue);
    expect(play.couldFinishIn, play.round.fewest);
  });
