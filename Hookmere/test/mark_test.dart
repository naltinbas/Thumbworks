...
import 'package:hookmere/ui/mark.dart';
import 'package:hookmere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real staircase, and checked', () {
    // The picture is 4, 2, 1, 1: the staircase of eight boxes with the
    // most fillings there are, 90 of them.
    final mark = Mark.shape;
    expect(mark.rows, [4, 2, 1, 1]);
    expect(Rules.valid(mark.rows), isTrue);
    expect(mark.hooks, [7, 4, 2, 1, 4, 1, 2, 1]);
    expect(mark.hookProduct, 448);
    expect(mark.byHooks, 90);
    expect(mark.counted, 90);
    expect(mark.level.meets(mark.rows), isTrue);
  });
