...
import 'package:skeinwell/ui/mark.dart';
import 'package:skeinwell/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real village, and checked', () {
    // The picture is the ring through all five greens: it strings five
    // ways, every lane taking four of them, and five lots of 4/5 make
    // the four the theorem promises.
    final play = Mark.village;
    expect(Rules.laidLanes(play.village), [0, 3, 4, 7, 9]);
    expect(Rules.joinedUp(play.village), isTrue);
    expect(play.stringings, 5);
    expect(play.shares.values.map((s) => '$s').toSet(), {'4/5'});
    expect('${play.total}', '4');
    expect(play.level.meets(play.village), isTrue);
  });
