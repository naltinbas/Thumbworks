...
/// One ask, the purses and the coin set to it.
class DuelScreen extends StatefulWidget {
  const DuelScreen({super.key, required this.level});

  final Level level;

  @override
  State<DuelScreen> createState() => DuelScreenState();
}
...
class DuelScreenState extends State<DuelScreen> {
  ...
  String verdict() {
    ...
    if (play.gaveUp) return 'Six coins to one against the coin is as near as it comes, 63/127: 2 to the pot less 1 is odd, and never twice anything.';
    final head = 'Ash ${play.ash} to Birch\'s ${play.birch}, the coin ${Rules.coinNames[play.coin]}: Ash\'s chance of the pot is ${play.chance}, '
        'and the duel lasts ${play.lasts} toss${play.lasts == Frac.one ? '' : 'es'} on average.';
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Text(
                  'Fill the purses a coin a tap, turn the coin over, and see '
                  'Ash\'s chance of the pot: ${widget.level.task}.',
                  style: const TextStyle(
              ...
              if (!play.isOver)
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      _dial(0, 'Ash', play.ash),
                      const SizedBox(width: 8),
                      _dial(1, 'Birch', play.birch),
                    ],
                  ),
                ),
              if (!play.isOver)
                OutlinedButton(
                  ...
                  child: Text('The coin ${Rules.coinNames[play.coin]}, ${Rules.coins[play.coin]}'),
                ),
              if (!play.isOver)
              Wrap(
                spacing: 8,
