...
import 'package:pursebury/ui/mark.dart';
import 'package:pursebury/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the sag of six to one against the coin, real and checked', () {
    // The picture is Ash's six coins to Birch's one with the coin against
    // him: 63/127, under a half, the nearest the hopeless ask comes.
    final play = Mark.sag;
    expect((play.ash, play.birch, play.coin), (6, 1, 0));
    expect(play.chance, Frac.of(63, 127));
    expect(play.chance.compareTo(Frac.of(1, 2)), lessThan(0));
    expect(play.chain.last, Frac.one);
