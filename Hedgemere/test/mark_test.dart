...
import 'package:hedgemere/ui/mark.dart';
import 'package:hedgemere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real hedge, and checked', () {
    // The picture is post 1 holding two posts and each of those
    // holding two more: four steps end to end, so one post is left
    // standing and it takes two rounds to get to it.
    final play = Mark.hedge;
    expect(play.hanging, [1, 2, 2, 3, 3]);
    expect(Rules.validHanging(play.hanging), isTrue);
    expect(play.middle, [1]);
    expect(play.rounds, 2);
    expect(play.longest, 4);
    expect(play.level.meets(play.hanging), isTrue);
  });
