...
import 'package:carterfen/ui/mark.dart';
import 'package:carterfen/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the shortest round on that map', () {
    // Driven through the same code a finger goes through, so the picture is a
    // round the game would call finished, and the shortest one there is.
    final play = Mark.driven;
    expect(play.isDone, isTrue);
    expect(play.isShortest, isTrue);
    expect(play.length, Mark.round.shortest);
  });
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onGround: false),
        ),
      );
    }
