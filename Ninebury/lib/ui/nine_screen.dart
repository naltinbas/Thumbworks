...
/// One ask, the three dials set to it.
class NineScreen extends StatefulWidget {
  const NineScreen({super.key, required this.level});

  final Level level;

  @override
  State<NineScreen> createState() => NineScreenState();
}

class NineScreenState extends State<NineScreen> {
  late Play play;
  ...
  String verdict() {
    ...
    final n = play.number;
    if (play.gaveUp) return '$n is ${_sqrt(n)} times ${_sqrt(n)}, root ${play.root}: a square roots 1, 4, 7 or 9, never five.';
    if (n == 0) return 'Nought on the dials: root 0, and no nines to cast.';
    final (nines, over) = Rules.cast(n);
    final head = '$n: ${Rules.told(n)}, root ${play.root}; $nines nine${nines == 1 ? '' : 's'} and $over over.';
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Text(
                  'Turn the three dials, a step a tap, and watch the digits '
                  'walk the nine-hour face: ${widget.level.task}.',
                  style: const TextStyle(
              ...
              if (!play.isOver)
                Padding(
                  ...
                  child: Wrap(
                    ...
                    children: [
                      _dial(0, 'hundreds', play.digits[0]),
                      _dial(1, 'tens', play.digits[1]),
                      _dial(2, 'units', play.digits[2]),
                    ],
                  ),
                ),
              if (!play.isOver)
                ...
                children: [
                  Chip(
                    ...
                    label: Text(
                      'root ${play.root}',
                      style: const TextStyle(
                          color: Palette.root, fontSize: 13),
                    ),
                  ),
                  Chip(
                    backgroundColor: Palette.board,
                    side: BorderSide(
                        color: play.isDone ? Palette.good : Palette.line),
                    label: Text(
                      'digits add to ${Rules.digitSum(play.number)}',
                      style: const TextStyle(
