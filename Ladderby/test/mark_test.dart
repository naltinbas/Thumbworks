...
import 'package:ladderby/ui/mark.dart';
import 'package:ladderby/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the middle rung, real and checked', () {
    // The picture is bottom 0, 3, 6 with top 1, 4, 7, the bottom three
    // shifted along by one: the crossings on the middle rung, in a line.
    final play = Mark.middle;
    expect(play.bottom, [0, 3, 6]);
    expect(play.top, [1, 4, 7]);
    final (x, y, z) = play.crossings!;
    expect([Rules.tell(x), Rules.tell(y), Rules.tell(z)], ['(2, 3)', '(7/2, 3)', '(5, 3)']);
    expect(play.crossingsByForm, play.crossings);
