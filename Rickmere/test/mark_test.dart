...
import 'package:rickmere/ui/mark.dart';
import 'package:rickmere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real field, and checked', () {
    // The picture is a three by four field with a square corner, its
    // ricks raised outward and the even ring of markers over them.
    final mark = Mark.field;
    expect(mark.posts, [(0, 0), (3, 0), (0, 4)]);
    expect(Rules.isField(mark.posts), isTrue);
    expect(mark.squareCorner, isTrue);
    expect(mark.halfAcres, 12);
    expect(mark.even, isTrue);
    final sides = mark.markerSides;
    expect(sides[0], sides[1]);
    expect(sides[1], sides[2]);
    expect(mark.level.meets(mark.posts), isTrue);
  });
