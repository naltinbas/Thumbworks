import 'package:flutter_test/flutter_test.dart';
import 'package:mintcombe/coin/levels.dart';

import 'support/fonts.dart';
import 'support/mintland.dart';

/// The sham, as first seen.
void main() {
  setUpAll(useRealFonts);

  testWidgets('the sham lists every ask by name and task',
      (tester) async {
    await open(tester);
    expect(find.text('Mintcombe'), findsOneWidget);
    for (final level in Levels.all) {
      expect(find.text(level.name), findsOneWidget);
      expect(
        find.textContaining(level.task.substring(1)),
        findsWidgets,
      );
    }
  });

  testWidgets('an ask opens from its tile', (tester) async {
    await open(tester);
    await tester.tap(find.text('The Ninety'));
    await tester.pumpAndSettle();
    expect(
      find.textContaining('Tap a coin on the rack to lay it on the counter'),
      findsOneWidget,
    );
  });

  testWidgets('a payment writes its fewest onto the sham',
      (tester) async {
    await open(tester);
    await tester.tap(find.text('The Ninety'));
    await tester.pumpAndSettle();
    await lay(tester, [89, 1]);
    expect(state(tester).play.isDone, isTrue);
    await press(tester, 'The sham');
    expect(find.textContaining('Fewest: 2'), findsOneWidget);
  });
}
