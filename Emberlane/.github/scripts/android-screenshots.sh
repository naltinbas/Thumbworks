#!/usr/bin/env bash
#
# Plays a run on the emulator the runner has already booted, and keeps the
# pictures.
#
# Two kinds of picture come out of a run. The ones the test asks the framework
# for are the point: each is taken at a chosen moment in a run the test
# played itself, so the one that matters has a wave part way down the lane
# with towers firing at it. The ones sampled off the
...
package=dev.emberlane.emberlane
