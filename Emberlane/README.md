...
- `field.dart` holds the map. One lane, laid out by hand rather than generated: a
  ...
  places in it.
- `kinds.dart` holds the four walkers and the three towers, and their numbers.
- `waves.dart` holds twenty waves, written down. The shape of a defence game
  is the order its problems arrive in: runners before there is anything quick
  enough to catch them, a lumberer just as the sparks that have been working
  start to look weak. A generator produces a list with no such moments.
- `run.dart` is the simulation, a sixtieth of a second at a time.
- `plan.dart` is a way of playing, written down, so the game can be played
...
twelve hundred embers in hand. It was measuring the schedule rather than the
game; orders are now a queue bought as fast as the embers come in. And an order that
...
**A tower shoots whichever walker in reach is furthest down the lane.** That
is the one about to get out, which is the one worth shooting and the one a player
