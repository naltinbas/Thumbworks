...
import 'package:turnstead/ui/mark.dart';
import 'package:turnstead/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the wheel\'s first round, strung full and fair', () {
    // The pairings are the wheel's own, made through the same code a
    // finger goes through: one full round, everyone playing.
    final play = Mark.strung;
    expect(play.rounds, hasLength(1));
    expect(play.rounds.single, hasLength(4));
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onPavilion: false),
        ),
      );
    }
