...
import 'package:ferrydale/ui/mark.dart';
import 'package:ferrydale/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the keeper one crossing over', () {
    // The pictured river goes through the same code a finger goes
    // through, so the logo is a live game, not an arrangement.
    var play = Mark.oneOver;
    expect(play.crossings, 1);
    expect(play.fewestFromHere, 6);
    var guard = 0;
    while (!play.isDone && guard++ < 10) {
      for (final who in play.nextLoad!) {
        play = play.board(who);
      }
      play = play.row();
    }
