...
import 'package:beatstow/ui/mark.dart';
import 'package:beatstow/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a laying that really juggles', () {
    // The five throws of the second ask, laid so that no two balls come
    // down together, which is one of the fifteen layings that do.
    final mark = Mark.laying;
    expect(Rules.juggles(mark.laid), isTrue);
    expect(Rules.total(mark.laid), 15);
    expect(mark.aloft.toSet(), {3});
    expect(mark.level.meets(mark.laid), isTrue);
    expect(Rules.landings(mark.laid).toSet().length, Rules.beats);
  });
