...
import 'package:arrowmere/ui/mark.dart';
import 'package:arrowmere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is a real orientation of the house, and it goes round', () {
    // The picture is the house pointed all the way the village lists it,
    // one of the six orientations that leave every place reachable from
    // every other.
    final play = Mark.round;
    expect(play.arrows, [false, false, false, false, false, false]);
    expect(play.roundabout, isTrue);
    expect(play.isDone, isTrue);
    expect(play.pairs, 20);
    expect(Rules.strong(Rules.house, play.arrows), isTrue);
    expect(Rules.bridges(Rules.house), isEmpty);
  });
