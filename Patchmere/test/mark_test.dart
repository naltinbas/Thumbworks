...
import 'package:patchmere/ui/mark.dart';
import 'package:patchmere/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the two by six sewn out by mirrors, real and checked', () {
    // The picture is the game against the house on the two-by-six:
    // three house patches, each answered across the middle, and the
    // quilt sewn out.
    final play = Mark.mirrored;
    expect(play.isOver, isTrue);
    expect(play.patches, hasLength(6));
    for (var i = 0; i < 6; i += 2) {
      expect(play.patches[i + 1].$1, play.quilt.mirror(play.patches[i].$1));
    }
  });
