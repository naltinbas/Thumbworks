...
void main() {
  ...
  testWidgets('the sham lists every ask by name and task',
      (tester) async {
    await open(tester);
    expect(find.text('Squarewell'), findsOneWidget);
  ...
  testWidgets('an ask opens from its tile', (tester) async {
    await open(tester);
    await tester.tap(find.text('The Two of Seven'));
    await tester.pumpAndSettle();
    expect(
      find.textContaining('Turn the base dial, a step a tap, and watch where the base and its opposite square to'),
      findsOneWidget,
    );
  });

  testWidgets('a square writes its fewest onto the sham',
      (tester) async {
    await open(tester);
    await tester.tap(find.text('The Two of Seven'));
    await tester.pumpAndSettle();
