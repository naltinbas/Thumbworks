...
/// The fewest presses each board has been put out in.
///
...
class Best {
  ...
  int? pressesFor(String board) => _prefs.getInt(_key(board));

  bool has(String board) => pressesFor(board) != null;

  int get done =>
      _prefs.getKeys().where((key) => key.startsWith(_prefix)).length;

  /// Writes down a board, and says whether it beat what was there.
  Future<bool> record(String board, int presses) async {
    final before = pressesFor(board);
    if (before != null && before <= presses) return false;
    await _prefs.setInt(_key(board), presses);
    return true;
  }
}
