#!/usr/bin/env bash
#
# Works a yard on the emulator the runner has already booted, and keeps the
# pictures.
#
# Two kinds of picture come out of a run. The ones the test asks the framework
# for are the point: each is taken at a chosen moment of a yard the test
# worked itself, so the one that matters is a yard it finished in par.
