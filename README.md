...
The world is built out of 280 stretches, and each one was played to the end —
by a search, before it ever went in the list.
...
been. What comes back is the list of steps to hold on — which is a proof,
...
- `ground.dart` — a stretch of world. Three kinds of tile and no fourth: flat,
  ...
  an hour.
- `runner.dart` — the physics, a hundred and twentieth of a second at a time.
  No randomness anywhere.
- `passable.dart` — the search, and `playWith`, which replays a proof.
- `maker.dart` — draws candidates and throws away the ones that cannot be got
  through, and the ones nobody has to jump on.
- `journey.dart` — stretches joined end to end, getting harder the further
  the run goes.
...
away — rounding it away is how a jump clears a gap on one phone and not
...
**The title plays itself**, driven by the stored proofs — the same trick the
tests use. What is running behind the words is a real run that nothing is
faking.
...
then the search: that it gets through what can be got through, that it needs no
