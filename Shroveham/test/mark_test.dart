...
import 'package:shroveham/ui/mark.dart';
import 'package:shroveham/ui/palette.dart';
...
void main() {
  ...
  test('the mark is the tall order three good flips in', () {
    // Every flip follows the walk through the same code a finger goes
    // through, so the picture is a batch still on course for its fewest.
    final play = Mark.threeIn;
    expect(play.made, 3);
    expect(play.couldStillBe, play.batch.fewest);
    ...
    for (final density in densities.entries) {
      ...
      await drawTo(
        ...
        Padding(
          ...
          child: const Mark(onHearth: false),
        ),
      );
    }
