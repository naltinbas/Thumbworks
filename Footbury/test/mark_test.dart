...
import 'package:footbury/ui/app.dart';
import 'package:footbury/ui/mark.dart';
import 'package:footbury/ui/palette.dart';
...
void main() {
  ...
  test('the mark is Simson\'s line, real and checked', () {
    // The picture is the triangle (5, 0), (-4, 3), (-3, -4) with the rim
    // point (0, 5): its feet at (-21/5, 22/5), (3, -1) and (-1, 2), in a
    // line.
    final play = Mark.simson;
    expect(play.corners, [(5, 0), (-4, 3), (-3, -4)]);
    expect(play.point, (0, 5));
    expect(play.feet!.map(Rules.tellPoint).toList(), ['(-21/5, 22/5)', '(3, -1)', '(-1, 2)']);
    expect(play.line, isNotNull);
    expect(play.ratio, Frac.zero);
